...
export class SNodeService {
  ...
  getStuMultiple(course_id: string, mindmap_id: string, node_id: string, stu_name: string): Observable<StuMultiple[]> {
    ...
    // 学生获取推荐题列表
    getRecommendMulti(course_id: string, mindmap_id: string, node_id: string, stu_name: string): Observable<StuMultiple[]> {
        this.tempUrl = this.baseUrl + 'recommend_multi/' + course_id + '/' + mindmap_id + '/' + node_id + '/' + stu_name;
        return this.http.get<StuMultiple[]>(this.tempUrl);
    }
