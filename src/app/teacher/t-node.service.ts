...
export class TNodeService {
  ...
  // 获取学生对应结点的题目完成情况
  getFinishedMultiples(course_id: string, mindmap_id: string, node_id: string, student_name: string): Observable<FinishedMultiple[]> {
    this.tempUrl = this.baseUrl + 'finished_multiples/' + course_id + '/' + mindmap_id + '/' + node_id + '/' + student_name;
    return this.http.get<FinishedMultiple[]>(this.tempUrl);
  }
    getFinishedJudges(course_id: string, mindmap_id: string, node_id: string, student_name: string): Observable<FinishedJudge[]> {
        this.tempUrl = this.baseUrl + 'finished_judges/' + course_id + '/' + mindmap_id + '/' + node_id + '/' + student_name;
        return this.http.get<FinishedJudge[]>(this.tempUrl);
    }
    getFinishedShorts(course_id: string, mindmap_id: string, node_id: string, student_name: string): Observable<FinishedShort[]> {
        this.tempUrl = this.baseUrl + 'finished_shorts/' + course_id + '/' + mindmap_id + '/' + node_id + '/' + student_name;
        return this.http.get<FinishedShort[]>(this.tempUrl);
    }
  ...
  removeShort(id: number) {
    this.tempUrl = this.baseUrl + 'delete_short/' + id;
    return this.http.delete<any>(this.tempUrl, httpOptions);
  }
  removeJudge(id: number) {
    this.tempUrl = this.baseUrl + 'delete_judge/' + id;
    return this.http.delete<any>(this.tempUrl, httpOptions);
  }
  removeMultiple(id: number) {
    this.tempUrl = this.baseUrl + 'delete_multiple/' + id;
    return this.http.delete<any>(this.tempUrl, httpOptions);
  }
}
